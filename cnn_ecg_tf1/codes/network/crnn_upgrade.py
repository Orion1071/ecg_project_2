    ...
    def create_model(self, data):
        ...
        with tf.compat.v1.name_scope('convolutions'):
            for i in range(self.n_conv_blocks):
                with tf.compat.v1.name_scope('conv2d_layer' + str(i)):
                    n_channels = None
                    if i == 0:
        ...
        with tf.compat.v1.name_scope('gradient_diode'):
            feature_seq = tf.cond(pred=tf.equal(self.training_phase, 1), # when in phase 1 do only train RNN
                           true_fn=lambda: tf.stop_gradient(feature_seq),
                           false_fn=lambda: tf.identity(feature_seq))

        with tf.compat.v1.name_scope('model_selector'):
            features = tf.cond(pred=tf.equal(self.training_phase, 0), # when in phase 0 do not use RNN
                           true_fn=lambda: nn.mean_branch(feature_seq, length, self.n_lstmneurons),
                           false_fn=lambda: nn.lstm_layer(feature_seq, length, self.n_lstmneurons, self.n_lstmlayers, bidirectional=self.bidirectional, drop_rate=self.drop_rate))
