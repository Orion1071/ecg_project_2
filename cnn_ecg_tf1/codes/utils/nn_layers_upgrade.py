...
def conv2d_block(inputs, length, kernel_size=[3,3], n_channels=None, growth=0, dilation_rates=[1,1,1],
    ...
    for d in range(depth):
        ...
        conv = tf.compat.v1.layers.batch_normalization(
                    inputs=conv,
                    center=True,
                    scale=True,
        ...
        if max_pool_en:
            conv = tf.compat.v1.layers.max_pooling2d(
                    inputs=conv, 
                    pool_size=strides_end, 
                    strides=strides_end, 
                    padding='same')
        output = tf.compat.v1.layers.dropout(inputs=conv, rate=drop_rate)

    length = tf.math.floordiv((length+1),2)
...
def awgn_channel(input, snr):
    ...
    if dim==2:
        e = tf.reduce_sum(input_tensor=e, axis=1)
    else:
        e = tf.reduce_mean(input_tensor=e, axis=2)
        e = tf.reduce_sum(input_tensor=e, axis=1)
    # average power of input signal (neglecting zero padding in division)
    p = tf.compat.v1.div(e, tf.cast(l, tf.float32))
    snr = tf.constant(snr, tf.float32)
    stddev = tf.sqrt(tf.compat.v1.div(p, snr))
    # make 3d
    stddev = tf.expand_dims(stddev, 1)
    stddev = tf.expand_dims(stddev, 1)
    # generate noise of same shape
    noise = tf.random.normal( shape, mean=0.0, stddev=1, dtype=tf.float32)
