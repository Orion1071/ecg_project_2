...
def create_variable(name, shape):
    var = tf.compat.v1.Variable(tf.random.normal(shape), name=name, trainable=True)
    tf.compat.v1.add_to_collection('vars', var)
    return var

def create_input(dtype, shape, name):
    init_name = '_'.join([name,'init'])
    init = tf.compat.v1.placeholder(dtype, shape, name=init_name)
    cast = tf.cast(init, gpu_prec)
    var = tf.compat.v1.Variable(cast, name=name, trainable=False, collections=[], validate_shape=False)
    var.set_shape(shape)
    tf.compat.v1.add_to_collection('inputs', var)
    tf.compat.v1.add_to_collection('inputs', init)
    return [init, var]
...
def get_dynamiclast(output, seq_l):
    rng = tf.range(0, tf.shape(input=seq_l)[0])
...
def set_dynamiczero(input, length):
    ...
    output = tf.compat.v1.where(mask, input, tf.zeros(shape, dtype=gpu_prec))
    return output
...
def get_length(sequence):
    ...
    ranged =  rng * used
    length = tf.reduce_max(input_tensor=ranged, axis=1) + 1
    return length
...
def where_greater(input, threshold, replacement):
    ...
    output = tf.compat.v1.where(condition, input, rep)
    return output

def get_prediction(net_output):
    prediction =  tf.argmax(input=net_output, axis=1)
    return prediction
...
def truncate_dynamic(data):
    ...
    print(length)
    size = tf.reduce_max(input_tensor=length)
    trunc = data[:,0:size]
    return trunc
...
def compute_score(prediction, label):
    ...
    f1 = tf.compat.v1.where(not_nan, f1, [0,0,0,0])
    f = tf.reduce_sum(input_tensor=f1)/tf.reduce_sum(input_tensor=tf.cast(not_nan, gpu_prec))
    return f
...
def calculate_F1(indexA, indexB, val):
    ...
    n1 = tf.reduce_sum(input_tensor=tf.cast(b1, gpu_prec))
    n2 = tf.reduce_sum(input_tensor=tf.cast(b2, gpu_prec))
    ncorrect = tf.reduce_sum(input_tensor=tf.cast(both_true, gpu_prec))
    f1 = 2*ncorrect/(n1 + n2)
    return f1
