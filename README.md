...
|2023 Jan 2nd week| Trained multiple models with smaller size, they all have 85% accuracy, 3 models done, having trouble with operation REDUCE_ANY not found, model size is not fitting on the ram, SPIRAM is not configured correctly, model is not fit in the memory, train 7 more models with different parameter|
|2023 Jan 3rd week| used menuconfig from espidf to configure spiram and other stuff, putting the model in flash instead of ram, SPIRAM working now, not enough space in flah, only seeing 1MB (Available is 8MB), used custom partition table for 8MB, still limited to 4MB, there must be something to do in partition table. model size of 2.5 MB will fit on the board|
|2023 Jan 4th week| run the model but working how to get around REDUCE_ANY op code, remove masking layer, it fixed the error. the model has accuracy 80% validation and 80% testing (on unseen data), used this model to test on the esp32. 1000 data points the model inference run 15 secs on computer , expected 66 hours on esp32, 1:15 mins to send data, 2:50 mins to run inference|
